...
resource "aws_db_instance" "orders_postgres_instance" {
  ...
  vpc_security_group_ids = [aws_security_group.rds_sg_orders.id]  # Link to security group
}

# Security group to allow inbound traffic on PostgreSQL port
resource "aws_security_group" "rds_sg_orders" {
  name        = "rds_public_access_sg_orders"
